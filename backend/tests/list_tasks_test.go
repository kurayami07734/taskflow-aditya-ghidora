...
func TestListTasksIntegration(t *testing.T) {
	...
	createTask := func(token, projectID, title, status string) string {
	...
	t.Run("list tasks returns 200", func(t *testing.T) {
		token := registerAndLogin("listtasks1@example.com", "secret123")
		projectID := createProject(token, "Project", "Description")
		createTask(token, projectID, "Task 1", "todo")
		createTask(token, projectID, "Task 2", "done")

		req, _ := http.NewRequest("GET", "/projects/"+projectID+"/tasks", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
	...
	t.Run("list tasks with status filter returns 200", func(t *testing.T) {
		token := registerAndLogin("listtasks2@example.com", "secret123")
		projectID := createProject(token, "Project", "Description")
		createTask(token, projectID, "Task 1", "todo")
		createTask(token, projectID, "Task 2", "todo")

		req, _ := http.NewRequest("GET", "/projects/"+projectID+"/tasks?status=todo", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
	...
	t.Run("list tasks on another users project returns 403", func(t *testing.T) {
		token1 := registerAndLogin("listtasks3a@example.com", "secret123")
		token2 := registerAndLogin("listtasks3b@example.com", "secret123")
	...
	t.Run("list tasks on non-existent project returns 404", func(t *testing.T) {
		token := registerAndLogin("listtasks4@example.com", "secret123")

		req, _ := http.NewRequest("GET", "/projects/00000000-0000-0000-0000-000000000000/tasks", nil)
		req.Header.Set("Authorization", "Bearer "+token)
		rr := httptest.NewRecorder()
	...
	t.Run("list tasks without auth returns 401", func(t *testing.T) {
		req, _ := http.NewRequest("GET", "/projects/some-id/tasks", nil)
		rr := httptest.NewRecorder()
